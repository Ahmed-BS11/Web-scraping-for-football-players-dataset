...
with open('DebutFin2019_2020.csv', 'a', newline='', encoding='utf8') as file:
        ...
        for num_page in range(56, 81):
                ...
                for name, key in zip(range(len(names)), range(len(keys))):
                    url = "https://www.transfermarkt.com/" + str(names[name]).replace('.', '-') + "/leistungsdatendetails/spieler/" + str(
                        keys[key]) + "/saison/2019/verein/0/liga/0/wettbewerb//pos/0/trainer_id/0/plus/1"
